...
"Penalize" primarily penalizes the mouse from potentialy backtracking and looping – the 4 P:s. It's better suited for a more open-spaced maze, but horrendously shitty when the corridors are more long-winded. 
 
 "Encouraging" encourages a more goal-oriented approach rather than the exploratory one in "Penalize". Infortunatly, it has the habit of investigating every single nook and cranny. Though, that err could be remedied by combining elements of "Penalize" and "Encouraging" into one "amalgamated" code. 
